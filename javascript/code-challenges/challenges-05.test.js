...
const addValues = (arr) => {
  // Solution code here...
  let add = arr.reduce((value, index) => {
    value += index;
    return value;
  }, 0)
  return add;
...
const addPurchases = (arr) => {
  let totalprice = arr.reduce((add, index) => {
    add += index.purchasePrice;
    return add;
  }, 0);
  return totalprice;
};
...
const countNumberOfElements = (arr) => {
  ...
  let elementsNo = arr.reduce(((add, index) => index ? add + 1 : add + 0), 0)
...
const returnNames = (arr) => {
  let onlyName = arr.reduce((add, index) => {
    add.push(index.name);
    return add;
  }, []);
  return onlyName;
...
const reversedString = (str) => {
  let array = str.split('');
  let reverse = array.reduce((add, index) => {
    return index + add;
  }, '');
  return reverse;
