...
function transformToLis(obj){
  return Object.entries(obj).map(i => `<li>${i.join(': ')}</li>`);
}
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(newArray => newArray.filter(value => (typeof value === 'number' && value % 5 === 0)).map(value1 => Math.pow(2, value1)));
};
