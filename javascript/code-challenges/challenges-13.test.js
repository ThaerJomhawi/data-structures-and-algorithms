...
const firstLetters = (arr) => arr.map((element) => element.substring(0, 1));
...
const findHappiness = (arr) => {
  let res = arr.filter((val) => val.includes(":)"));
  return res;
};
...
const onlyOddChars = (str) => str.split('').filter(element => str.indexOf(element) % 2).join('');
